...
*/
function repeatedPOST(url, body, successConditions, failureConditions, interval=1, timeout=10, options={headers: {'Accept':'application/json'}, validateStatus : undefined}){
        ...
        const _interval = setInterval(async ()=>{
            ...
            try{
                ...
                if(successConditions(response)){
                    ...
                    // console.log("AYYYYY! "+response.status+" GOIN OUT, CMON! "+response.data.error)
                    end();
                    reject(response);
                } 
                // else {
...
class DeviceFlowManager {
    ...
    */
    tokenRequest(authorizationResponse, clientid, clientsecret=undefined){throw new Error("Not implemented.");}
    ...
    * @returns {Promise<Object>} The response data.
    */
    authorizationRequest(clientid, scopes){throw new Error("Not implemented.");}
}
...
class GoogleDeviceFlow extends DeviceFlowManager {
    ...
    */
    authorizationRequest(clientid, scopes){
        return axios.get("https://accounts.google.com/.well-known/openid-configuration", {headers:{'Accept':'application/json'}}
        ).then(response=>{
            //OK
